...
def store_gemme_as_h5(
    gemme_folder: Path,
    output_file: Path
) -> None:
    with h5py.File(output_file, 'w') as hdf:
        for file in progress.track(list(gemme_folder.glob("*_normPred_evolCombi.txt")), description=f"Loading GEMME score files from {gemme_folder}"):
