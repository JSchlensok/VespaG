...
from vespag.utils.utils import GEMME_ALPHABET, normalize_scores
...
def test_mask_non_mutations() -> None:
    ...
    torch.testing.assert_close(
        pred[list(range(len(wildtype_sequence))), [GEMME_ALPHABET.index(aa) for aa in wildtype_sequence]],
        torch.zeros(3),
    )
